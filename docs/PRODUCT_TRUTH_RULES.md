...
内部枚举与状态机名不得公开;公开映射唯一来源 `src/report/presentation/zh-labels.ts`
(实测/公开网页估算/证据不足/暂未测得;直接支持/部分支持/背景参考;UNSUPPORTED
不公开)。估算权重高于实测时必须显著提示估算性质。机会必须携带来源 Issue、
证据与优先理由;被剪枝候选记录 reasonCode,且剪枝原因不得公开为企业问题。
