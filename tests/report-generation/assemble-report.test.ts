...
describe("buildReportFromStageOutputs → assembleReport → DiagnosisReport.parse", () => {
  ...
  it("is shape-compatible with the shared SAMPLE_DIAGNOSIS_REPORT anchor", () => {
    ...
    // Note: questionCoverageGaps is optional in DiagnosisReport and not generated by assembleReport
    // Generated report must be a subset of sample keys (optional fields may exist in sample but not generated)
    ...
    const missingFromGenerated = sampleKeys.filter(
      (k) => !generatedKeys.includes(k) && !["questionCoverageGaps"].includes(k)
    );
