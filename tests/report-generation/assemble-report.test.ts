...
describe("buildReportFromStageOutputs → assembleReport → DiagnosisReport.parse", () => {
  ...
  it("is shape-compatible with the shared SAMPLE_DIAGNOSIS_REPORT anchor", () => {
    ...
    // Note: questionCoverageGaps and questionCoverageAssessments are optional in DiagnosisReport
    // and not generated by assembleReport. Generated report must be a subset of sample keys
    // (optional fields may exist in sample but not generated).
    const generatedKeys = Object.keys(r).sort();
    const sampleKeys = Object.keys(SAMPLE_DIAGNOSIS_REPORT).sort();
    // All generated keys must exist in sample (no extra required keys)
    const extraInGenerated = generatedKeys.filter((k) => !sampleKeys.includes(k));
    expect(extraInGenerated).toEqual([]);
    // All required sample keys must exist in generated (optional fields are OK to be missing)
    const missingFromGenerated = sampleKeys.filter(
      (k) => !generatedKeys.includes(k) && !["questionCoverageGaps", "questionCoverageAssessments"].includes(k)
    );
    expect(missingFromGenerated).toEqual([]);
  });

  it("has matching score keys with the sample report", () => {
    const res = buildReportFromStageOutputs(BASE_INPUT);
    if (!res.ok) throw new Error("expected ok");
    const r = res.report;
